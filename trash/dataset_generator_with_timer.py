...
def generate_frame(data_dir, background, annotation_writer, frame_path, ds_type):
    start = time.time()
    random_scale = random.randint(75, 200) / 100
    ...
    while new_x < bg_width:
        ...
        foreground = cv2.imread(f'{data_dir}/{obj_name}/{img_name}')
        try:
            foreground = crop_and_convert(foreground)
            foreground = cv2.resize(foreground,
                                    (int(foreground.shape[1] * random_scale), int(foreground.shape[0] * random_scale)))
...
if __name__ == "__main__":
    ...
    with open(f'{NEW_PATH_PREFIX}/csv_annot.csv', 'a') as csv_annotation:
        ...
        for f_counter in range(1, FRAMES + 1):
            frame_path = f"{NEW_PATH_PREFIX}/{f_counter}.jpg"
            frame, f_time = generate_frame(SOURCE_PATH_PREFIX, background_img, annot_writer, frame_path,
                                           get_ds_type(f_counter))
            cv2.imwrite(frame_path, frame)
            avrg_time = (avrg_time * (f_counter - 1) + f_time) / f_counter
            time_left = int(avrg_time * (FRAMES - f_counter))
            print(f"frame {f_counter}: Done!")
            print(f"{time_left // 60} m., {time_left % 60} sec. ({FRAMES - f_counter} frames) left")
